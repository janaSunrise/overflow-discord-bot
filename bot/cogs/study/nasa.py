...
class Nasa(Cog):
    ...
    async def astronomy_picture(self, ctx: Context) -> None:
        ...
        embed = Embed(
            title=data["title"], description=description, color=Color.blurple()
        )
    ...
    async def nasa_id(self, ctx: Context, id: str) -> None:
        ...
        async with self.bot.session.get(
            f"https://images-api.nasa.gov/asset/{id}"
        ) as resp:
            data = await resp.json()
    ...
    async def nasa_patent(self, ctx: Context, *, patent: str) -> None:
        ...
        async with self.bot.session.get(
            f"https://api.nasa.gov/techtransfer/patent/?{patent}&api_key={NASA_API}"
        ) as resp:
            data = await resp.json()
    ...
    @command()
    async def mars(
        self, ctx: Context, date: str, rover: str = None, number: int = 1
    ) -> None:
        """
        Get images from Mars. must specify the date (in the form YYYY-MM-DD),
        and you can specify the rover and the number of images to retrieve.
        """
        if rover is None:
