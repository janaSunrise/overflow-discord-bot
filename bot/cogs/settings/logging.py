...
class LoggingSettings(Cog):
    ...
    @group(invoke_without_command=True, aliases=["log-config", "logging", "logchannel", "log"])
    ...
    async def server(self, ctx: Context, channel: TextChannelConverter = None) -> None:
        if not channel:
            await ctx.send(":x: Specify a channel to configure as the server log channel.")

        await Logging.set_log_channel(self.bot.database, "server_log", ctx.guild.id, channel.id)
    ...
    async def mod(self, ctx: Context, channel: TextChannelConverter = None) -> None:
        ...
        await Logging.set_log_channel(self.bot.database, "mod_log", ctx.guild.id, channel.id)
    ...
    async def message(self, ctx: Context, channel: TextChannelConverter = None) -> None:
        if not channel:
            await ctx.send(":x: Specify a channel to configure as the message log channel.")

        await Logging.set_log_channel(self.bot.database, "message_log", ctx.guild.id, channel.id)
    ...
    async def member(self, ctx: Context, channel: TextChannelConverter = None) -> None:
        if not channel:
            await ctx.send(":x: Specify a channel to configure as the member log channel.")

        await Logging.set_log_channel(self.bot.database, "member_log", ctx.guild.id, channel.id)
    ...
    async def join(self, ctx: Context, channel: TextChannelConverter = None) -> None:
        if not channel:
            await ctx.send(":x: Specify a channel to configure as the join log channel.")

        await Logging.set_log_channel(self.bot.database, "join_log", ctx.guild.id, channel.id)
    ...
    async def voice(self, ctx: Context, channel: TextChannelConverter = None) -> None:
        if not channel:
            await ctx.send(":x: Specify a channel to configure as the voice log channel.")

        await Logging.set_log_channel(self.bot.database, "voice_log", ctx.guild.id, channel.id)
